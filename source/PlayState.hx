...
class PlayState extends MusicBeatState
{
	...
	var funnyFloatyBoys:Array<String> = ['dave-3d', 'bambi-3d', 'bambi-3d-b', 'bambi-unfair', 'unfair_bambi-b', 'god_unfair_bambi', 'expunged', 'bambi-piss-3d', 'bambi-scaryooo', 'bambi-god', 'bambi-god2d', 'bambi-hell', 'bombu'];
	...
	override public function create()
	{
		...
		case '3dGreen-b':
			{
				defaultCamZoom = 0.85;
				curStage = '3dGreen-b';
				var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('dave/cheater-b'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.6, 0.6);
				bg.active = true;
		...
		case '3dScary':
                        ...
                        case '3dScary-b':
			{
				defaultCamZoom = 0.8;
				curStage = '3dScary-b';
				var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('dave/scarybg-b'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.6, 0.6);
				bg.active = true;
                        ...
                        case 'god2bg':
			{
				defaultCamZoom = 0.8;
				curStage = 'god2bg';
				var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.image('dave/god2bg'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.6, 0.6);
				bg.active = true;
		...
		if(SONG.song.toLowerCase() == "unfairness")
		{
			health = 2;
                        ...
                        if(SONG.song.toLowerCase() == "unfairness-b")
		{
			health = 2;
		}
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'unfairness':
				credits = "Ghost tapping is forced off! Screw you!";
                                    case 'unfairness-b':
				credits = "the key settings were foced reset! Screw you!";
                                    case 'god-unfairness-b':
				credits = " you're done.";
			case 'opposition':
	...
	if (SONG.song.toLowerCase() == 'unfairness-b') // fuck you x4
			{
				playerStrums.forEach(function(spr:FlxSprite)
				{
					spr.x = ((FlxG.width / 2) - (spr.width / 2)) + (Math.sin(elapsedtime + (spr.ID)) * 300);
	...
	public function moveCamera(isDad:Bool)
		{
			...
			if(isDad)
			{
				...
				switch (dad.curCharacter)
				{
					...
					case 'bambi-3d' | 'bambi-3d-b' | 'bambi-unfair' | 'unfair_bambi-b' | 'god_unfair_bambi':
			...
			if ((controlHoldArray.contains(true) || controlArray.contains(true)) && !endingSong) {
				...
				if (controlArray.contains(true)) {
					for (i in 0...controlArray.length) {
						...
						if(SONG.song.toLowerCase() == "god-unfairness-b")
								{
									canMiss = true;
									ghostMiss();
								}
